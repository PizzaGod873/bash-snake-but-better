...
getchar() {
    ...
    while true; do
        read -s -n 1 key
        if [[ $key == $'\x1b' ]]; then  # ESC detected - possibly arrow keys
            read -s -n 2 -t 0.1 rest
            case "$rest" in
                '[A') kill -$SIG_UP $game_pid ;;
                '[C') kill -$SIG_RIGHT $game_pid ;;
                '[B') kill -$SIG_DOWN $game_pid ;;
                '[D') kill -$SIG_LEFT $game_pid ;;
            esac
        else
            case "$key" in
...
game_loop() {
    ...
    echo -e "${text_color}Oh, No! You Died$no_color"
